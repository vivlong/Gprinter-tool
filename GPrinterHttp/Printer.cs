...
namespace GPrinterHttp
{
    public class Printer
    {
        ...
        public void StartPrint(object msg)
        {
            Logger.Debug("接收数据API:" + msg);
            try
            {
                if (NewUsb.USBState)
                {
                    ...
                    //SendData2USB($"TEXT 400,15,\"2\",0,1,1,\"15121540207\"\r\n"); //字符串
                    //SendData2USB(enddata);
                    //SendData2USB($"TEXT 400,15,\"2\",0,1,1,\"{msg}\"\r\n"); //字符串
                    //SendData2USB("TEXT 15,60,\"TSS24.BF2\",0,1,1,\"简体字\"\r\n"); //字符串
                    //SendData2USB($"QRCODE 400,40,H,4,A,0,\"{msg}\"\r\n"); //二维码
                    //SendData2USB($"QRCODE 400,40,H,4,A,0,\"{msg}\"\r\n"); //二维码
                    SendData2USB($"TEXT 50,120,\"3\",0,1,1,\"Order NO:39049-006\"\r\n"); //字符串

                    SendData2USB(enddata);
                    SendData2USB($"TEXT 50,120,\"3\",0,1,1,\"Order NO:39049-006\"\r\n"); //字符串
                    SendData2USB(enddata);
                    SendData2USB($"BAR 50,180,700,2\r\n");
                    SendData2USB(enddata);
                    SendData2USB($"TEXT 50,200,\"2\",0,1,1,\"Model\"\r\n"); //字符串
                    SendData2USB($"TEXT 250,200,\"2\",0,1,1,\"Unit\"\r\n"); //字符串
                    SendData2USB($"TEXT 450,200,\"2\",0,1,1,\"Qty\"\r\n"); //字符串
                    SendData2USB(enddata);
                    SendData2USB($"BAR 50,230,700,2\r\n");
                    SendData2USB(enddata);

                    // 循环体部分
                    SendData2USB($"TEXT 50,300,\"2\",0,1,1,\"ASD33C\"\r\n"); //字符串

                    // qrcode
                    SendData2USB($"TEXT 100,500,\"2\",0,1,1,\"FRONT\"\r\n"); //字符串
                    SendData2USB($"QRCODE 100,520,H,4,A,0,\"www.yishunbike.com\"\r\n");
                    SendData2USB($"TEXT 100,660,\"2\",0,1,1,\"www.yishunbike.com\"\r\n"); //字符串


                    SendData2USB($"TEXT 400,500,\"2\",0,1,1,\"REAR\"\r\n"); //字符串
                    SendData2USB($"QRCODE 400,520,H,4,A,0,\"www.yishunbike.com\"\r\n");
                    SendData2USB($"TEXT 400,660,\"2\",0,1,1,\"www.yishunbike.com\"\r\n"); //字符串


                    // footer
                    SendData2USB(enddata);
                    SendData2USB($"BAR 50,710,700,2\r\n");
                    SendData2USB(enddata);
                    SendData2USB($"TEXT 500,730,\"3\",0,1,1,\"Data:2021-6-15\"\r\n"); //时间字符串
                    SendData2USB(enddata);
                    SendData2USB($"BAR 50,760,700,2\r\n");
                    SendData2USB(enddata);
        ...
        /// 打印机模板写死
        /// </summary>
        public void PrintModel(string model, string sku)
        {

            string modelName = "D:\\print\\model.btw";
            btFormat = btApp.Formats.Open(modelName, false, "");
            btFormat.PrintSetup.IdenticalCopiesOfLabel = 1;

            btFormat.SetNamedSubStringValue("model", model);
            btFormat.SetNamedSubStringValue("qrcode", sku);
            btFormat.PrintOut(true, false);
        }

        public void PrintBeiNo(string model, string bno)
        {
             try
            {
             string modelName = "D:\\print\\bie.btw";
             btFormat = btApp.Formats.Open(modelName, false, "");
            }
            catch (Exception ex)
            {
               Logger.Debug("加载BarTender实例失败"+ex);
            }
            ...
            btFormat.SetNamedSubStringValue("model", model);
            btFormat.SetNamedSubStringValue("bno", bno);
            btFormat.PrintOut(true, false);
        }
      
      public void PrintLunZu(string letno, string model, string qty, string pair, string frontsn, string frontn, string rearn, string rearsn)
       {
               try
              {
               string modelName = "D:\\print\\lunzu.btw";
               btFormat = btApp.Formats.Open(modelName, false, "");
              }
              catch (Exception ex)
              {
              ...
              btFormat.SetNamedSubStringValue("letno", letno);
              btFormat.SetNamedSubStringValue("model", model);
              btFormat.SetNamedSubStringValue("qty", qty);
              btFormat.SetNamedSubStringValue("qty", pair);
              btFormat.SetNamedSubStringValue("frontsn", frontsn);
              btFormat.SetNamedSubStringValue("frontn", frontn);
              btFormat.SetNamedSubStringValue("rearn", rearn);
              btFormat.SetNamedSubStringValue("rearsn", rearsn);
              btFormat.PrintOut(true, false);
          }
